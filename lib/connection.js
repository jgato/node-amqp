...
>>>>>>> Fix to have more than one queue with the same name
};
...
<<<<<<< HEAD
      ...
      for (var queue in self.queues) {
        for (var channel in self.queues[queue])
          for (var index in self.queues[queue][channel].consumerTagOptions) {
            self.queues[queue][channel].consumerTagOptions[index]['state'] = 'closed';
          }
      }
=======
            // In order for our reconnection to be seamless, we have to notify the
...
Connection.prototype.queueClosed = function (name, channel) {
  if (this.queues[name] && this.queues[name][channel]) delete this.queues[name][channel];
